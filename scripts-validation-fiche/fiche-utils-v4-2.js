...
/* === MESSAGES (HTML, classes du thème sj-*) === */

  // Échappement commun aux deux messages : le contenu vient du formulaire membre
  // et du staff, il est réinjecté tel quel dans un post.
  ...
  FI.bbcodeDemande = function (d) {
    ...
    const metier = d.sans_emploi
      ...
      : `<span>${esc(d.lieu_metier)}</span> <span>${esc(d.societe)}</span> <span>${esc(d.emploi)}</span>`;
...
<d>Habitation</d> ${FI.habitationResume(d)}
  ...
  FI.bbcodeValidation = function (d, msgPerso, staffPseudo) {
    return `<div class="sj-fiche"><div class="h1"><h1>Fiche validée</h1></div>
<tw><span>Bienvenue parmi nous @"${d.pseudo}" !</span></tw>
<div class="sj-formgen"><div class="sj-formcol"><f4>Le mot du staff</f4>${msgPerso}</div></div>
</div>`;
  };

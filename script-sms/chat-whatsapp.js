/* ============================================================
   chat-whatsapp.js
   Transforme l'affichage d'un sujet du sous-forum WhatsApp
   en conversation type messagerie.

   Vérifié contre le template viewtopic_body fourni.
   - Ne touche à RIEN si le fil d'Ariane ne pointe pas vers
     CONFIG.FORUM_SLUG.
   - Déplace les vrais noeuds (avatar, message, liens
     Citer/Éditer..., réactions) : liens et handlers natifs
     de FA restent fonctionnels.
   - Injection asynchrone de FA gérée par polling.
   ============================================================ */
(function () {
  "use strict";
  ...
  var CONFIG = {
    ...
    POLL_INTERVAL: 400,
    POLL_MAX:      25,
    LS_PREFIX:     "tdlChatAuteur_" // localStorage, indexé par id du topic
  };
  ...
  function estPage1() {
    if (/\/t\d+p[1-9]/.test(location.pathname)) return false;     // /t79p25-...
    var m = location.search.match(/[?&]start=(\d+)/);              // ?start=25
    if (m && parseInt(m[1], 10) > 0) return false;
    return true;
  }

  /* Récupère le vrai lien "Répondre" natif (sinon reconstruit) */
  function urlReponse() {
  ...
  function decoupeDate(texte) {
    ...
    jour = jour.replace(/[\s\u00A0]*(?:\u00E0|-)[\s\u00A0]*$/i, "").trim(); // retire " à" / " -"
    return { jour: jour, heure: heure };
  }

  /* Texte de date = dernier <span> de .sj-post-date (= {POST_DATE_NEW}) */
  ...
  function construireBulle(post, idAuteur) {
    ...
    var lien   = dateEl ? dateEl.querySelector("a[href]") : null; // permalien du post
    ...
    /* méta : heure (permalien) + double-coche si envoyé */
    ...
    /* menu actions (Citer / Éditer / Supprimer / IP) */
    if (btns) {
    ...
    post.appendChild(row); /* .sj-postmsg reste la bande opaque */
  ...
  function injecterBarre(dernierPost) {
    ...
    bar.innerHTML =
      ...
      '<span class="cw-bar-input">' + TEXTES.placeholder + '</span>' +
      '<i class="fi fi-rr-clip cw-bar-ico"></i>' +
      '<span class="cw-send"><i class="fi fi-rr-paper-plane"></i></span>';
    dernierPost.parentNode.insertBefore(bar, dernierPost.nextSibling);
  }

  /* Masque la réponse rapide native (remplacée par la barre) */
  function masquerReponseRapide() {
    var ancre = $('a[name="quickreply"]'); // [MAJ] le form natif est le frère suivant
    if (ancre && ancre.nextElementSibling) ancre.nextElementSibling.style.display = "none";
  }
  ...
  function appliquer() {
    ...
    injecterEntete(posts[0]);
    injecterBarre(posts[posts.length - 1]);
    masquerReponseRapide();
    return true;
  }
